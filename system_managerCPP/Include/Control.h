...
private:
    ...
    std::optional<Eigen::Vector3d> _noise_sim;
    ...
    Eigen::Vector3d _accel_cmd_ned;
    std::unique_ptr<Logger> _control_logger;
    
    double _last_pitch_update;
    bool _finished_stationary_tracking;
    Eigen::Vector3d _current_pos_ned;
    Eigen::Vector3d _current_vel_ned;
    
    std::optional<Eigen::Vector3d> prev_los_distance;
    std::optional<Eigen::Vector3d> prev_los_ned_dir;
...
public:
    ...
    std::tuple<Eigen::Vector3d, Eigen::Vector3d, Quaternion> get_cmd(
        ...
        const std::optional<std::tuple<Eigen::Vector3d, Eigen::Vector3d, Eigen::Vector3d>>& headingDest = std::nullopt,
    ...
    void log_control_data(const Eigen::Vector3d& command, const Eigen::Vector3d& rpy_rate_cmd,
                          ...
                          int64_t current_ts, int counter,
                          const Eigen::Vector3d& est_tar_pos_ned = Eigen::Vector3d::Zero(),
                          const Eigen::Vector3d& vel_des_ned = Eigen::Vector3d::Zero(),
