...
int mavlink_to_zmq_init(mavlink_to_zmq_t* bridge, const char* log_dir) {
    ...
    // Start data thread
    if (pthread_create(&bridge->data_thread, NULL, mavlink_to_zmq_data_thread_func, bridge) != 0) {
...
void mavlink_to_zmq_cleanup(mavlink_to_zmq_t* bridge) {
    if (bridge == NULL) {
        return;
    }
    
    // Stop threads
    mavlink_to_zmq_stop(bridge);
...
bool mavlink_to_zmq_init_succeeded(mavlink_to_zmq_t* bridge) {
    return bridge != NULL && bridge->init_success;
}

void mavlink_to_zmq_stop(mavlink_to_zmq_t* bridge) {
    if (bridge == NULL) {
        return;
    }
...
int mavlink_to_zmq_init_mavlink(mavlink_to_zmq_t* bridge) {
...
void mavlink_to_zmq_listener_to_mavlink(mavlink_to_zmq_t* bridge, bool blocking, double timeout, 
    ...
    if (use_lock) {
        ...
        mavlink_to_zmq_parse(bridge, msg_type, msg_dict);
        if (apply_filter) {
            mavlink_to_zmq_filter_data(bridge, &bridge->current_data);
        }
...
// Filter data
void mavlink_to_zmq_filter_data(mavlink_to_zmq_t* bridge, flight_data_t* current_data) {
...
void mavlink_to_zmq_parse(mavlink_to_zmq_t* bridge, const char* msg_type, void* msg_dict) {
        ...
        if (heartbeat.custom_mode != 0) {
            ...
            if (heartbeat.custom_mode != prev_custom_mode_id) {
                ...
                if (current_time - last_mode_debug_time > 1.0 || mode_change_count <= 5) {
                    printf("mavlink_to_zmq: Mode change #%d: %d -> %d (OFFBOARD=393216, HOLD=50593792)\n",
...
void* mavlink_to_zmq_data_thread_func(void* arg) {
    mavlink_to_zmq_t* bridge = (mavlink_to_zmq_t*)arg;
    struct timespec ts;
    double out_time = 0.0;
    ...
    while (bridge->running) {
        ...
        mavlink_to_zmq_listener_to_mavlink(bridge, false, 0.0, true, true);
